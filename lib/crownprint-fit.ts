...
import { labelForState } from "./crownprint-code";
...
export function matchProducts(profile: CrownPrintProfile, catalog: FitCatalogProduct[]): FitResult {
  ...
  for (const product of catalog) {
    ...
    const matchClass = signals > 0 ? classify(score) : null;
    ...
    const caution = spec.cautions?.find((c) => c.when(profile))?.text;
    ...
    scored.set(product.slug, {
      ...
      why,
      need: spec.need,
      whenToUse: spec.whenOverrides?.find((o) => o.when(profile))?.text ?? spec.when,
      ...(caution ? { caution } : {}),
  ...
  if (bundle) {
    ...
    if (carrying.length >= 3) {
      ...
      scored.set(BUNDLE_SLUG, {
        ...
        matchClass: allStrong ? "strong" : "good",
        why: `Your CrownPrint points at ${carrying.length === 4 ? "all four" : "most"} of the core steps — cleanse, condition, hydrate, and seal. This is those four together as one routine.`,
        need: "The full four-step routine",
        whenToUse: "Cleanse with Lathyr every 7–10 days, deep condition with Uplyft on wash day, refresh with Hydrate between washes, and seal with Nourish 2–4 times a week.",
        keyIngredients: [],
