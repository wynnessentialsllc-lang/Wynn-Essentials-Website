...
// ---------------------------------------------------------------------------
// ROUTINE — a SEPARATE authority from matches.
//
...
//
// They are different questions with different authorities and they must never
// be merged. A product can be a direct match without being in the routine, and
// a routine step is not a formulation match — it carries no match class and no
// evidence, because building a regimen is not the same act as authorizing a
// product against a resolved need.
//
// ORDER IS HWL'S. Wynn preserves routine[].order exactly and never re-sorts by
// match class or by its own Wynn Method step. Those are Wynn's orderings of
// different things; applying either here would silently overwrite the sequence
// the shopper actually built.
// ---------------------------------------------------------------------------

/**
 * Routine build state. Unrecognized values resolve to "unavailable" rather than
 * to a usable state: a routine Wynn cannot understand must fail transparently,
 * never quietly render as built or as never-started.
 */
const ROUTINE_STATUS = new Map([
  ["built", "built"],
  ["complete", "built"],
  ["not_built", "not_built"],
  ["not built", "not_built"],
...
const asRoutineStatus = (v) => {
  const raw = str(v, 40);
  if (!raw) return undefined;
  return ROUTINE_STATUS.get(raw.trim().toLowerCase()) ?? "unavailable";
};

/**
 * One ordered routine step. `order` is the authority; everything else is
 * consumer-safe display text HWL supplies and Wynn renders verbatim.
 *
 * Note what is NOT read: matchClass and evidence. A routine step is placement,
 * not authorization, and letting one carry a class would make the two arrays
 * indistinguishable on the page — the exact separation this contract exists to
 * hold.
 */
const asRoutine = (v, maxItems = 12) => {
  if (!Array.isArray(v)) return [];
  const out = [];
  for (const entry of v) {
    if (!entry || typeof entry !== "object") continue;
    ...
    if (!productKey || !Number.isFinite(order)) continue;
    out.push({
      order,
      productKey,
      ...(str(entry.slot ?? entry.stage, 120) ? { slot: str(entry.slot ?? entry.stage, 120) } : {}),
      ...(str(entry.routineRole ?? entry.role, 160) ? { routineRole: str(entry.routineRole ?? entry.role, 160) } : {}),
      ...(str(entry.whenToUse, 300) ? { whenToUse: str(entry.whenToUse, 300) } : {}),
      ...(str(entry.frequency, 160) ? { frequency: str(entry.frequency, 160) } : {}),
      ...(str(entry.why, 400) ? { why: str(entry.why, 400) } : {}),
      ...(entry.isAccessory === true ? { isAccessory: true } : {}),
    });
    if (out.length >= maxItems) break;
  }
  // Sorted by HWL's own order field — preserving it, not imposing one.
...
export function normalizeMatchContext(input, options) {
  ...
  // Routine placement authority, gated on entitlement exactly like matches.
  const routineStatus = crownPrintPresent ? asRoutineStatus(raw.routineStatus) : undefined;
  const routine = crownPrintPresent && routineStatus === "built" ? asRoutine(raw.routine) : [];
