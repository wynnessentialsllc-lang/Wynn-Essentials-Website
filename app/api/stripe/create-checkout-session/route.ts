...
export async function POST(request: Request) {
  try {
    ...
    const qualifiesForFreeShipping = commerceConfig.freeShippingRateId !== null && (!!freeShippingCampaign || subtotalCents >= commerceConfig.freeShippingThresholdCents);
    ...
    const session = await getStripe().checkout.sessions.create({
      ...
      metadata: { cartId, internalOrderReference: `WE-${new Date().getUTCFullYear()}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`, source: "wynn-essentials-website", invitationAccepted: String(Boolean(body.invitationAccepted)), ...(freeShippingCampaign ? { freeShippingCampaign: freeShippingCampaign.id } : {}), ...(body.routineRecommendationId ? { routineRecommendationId: body.routineRecommendationId.slice(0, 100) } : {}) },
    });
