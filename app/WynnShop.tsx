...
function Cart({ items, setItems, onClose, add, suggest }: { items: CartItem[]; setItems: (x: CartItem[]) => void; onClose: () => void; add: (p: Product) => void; suggest: Product[] }) {
  ...
  return <ModalShell label="Shopping bag" onClose={onClose} className="drawer-shell"><aside className="drawer">
    ...
    {!items.length ? <div className="empty"><p>Your bag is ready for an intentional routine.</p><button className="button" onClick={onClose}>Continue Shopping</button></div> :
      <>{detailed.map(({ product, quantity, color, variantId }) => { const v = product.variants?.find(vv => vv.id === variantId); const unit = v?.price ?? product.price; return <div className="cart-line" key={`${product.slug}-${color ?? ""}-${variantId ?? ""}`}><ProductArt product={product} small /><div><b>{product.name}</b><span>{product.subtitle}</span>{color && <span>Color: {color}</span>}{v ? <span>{v.length}{v.color ? ` · ${v.color}` : ""}</span> : <span>{product.size ?? "Size to be confirmed"}</span>}<div className="quantity"><button onClick={() => change(product.slug, color, variantId, -1)} aria-label={`Decrease ${product.name}`}>−</button><span>{quantity}</span><button onClick={() => change(product.slug, color, variantId, 1)} aria-label={`Increase ${product.name}`}>+</button></div><button className="remove" onClick={() => remove(product.slug, color, variantId)}>Remove</button></div><strong>{money(unit == null ? null : unit * quantity)}</strong></div>; })}
