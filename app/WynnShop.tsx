...
function FooterInfo({ page, onClose }: { page: FooterInfoKey; onClose: () => void }) {
  const content: Record<FooterInfoKey, { title: string; body: React.ReactNode }> = {
    ...
    shipping: { title: "Shipping Information", body: <><p>Orders may require up to 3 business days for processing before shipment. Delivery estimates and available rates are shown at checkout.</p><p>Please review your shipping address carefully. Address corrections, returned packages, and reshipments may result in additional charges. When your order ships, tracking information is sent to the email used at checkout.</p><p>If a package is marked delivered but cannot be found, contact the carrier first to request a trace, then contact Wynn Essentials with your order number.</p></> },
...
export default function WynnShop() {
  ...
  return <div className="site">
    ...
    <footer id="footer">
      ...
      <div><h3>Help</h3>{([["Contact","contact"],["Shipping","shipping"],["Returns","returns"],["FAQ","faq"],["Track Order","track"],["Accessibility","accessibility"]] as [string,FooterInfoKey][]).map(([label,key])=><button className="footer-link" key={key} onClick={()=>setFooterInfo(key)}>{label}</button>)}</div>
      ...
      <div className="footer-bottom"><button className="text-button" onClick={()=>setInvitation("manual")}>View Invitation</button><div><a href="https://www.instagram.com/wynnessentials/" target="_blank" rel="noreferrer">Instagram</a><a href="mailto:wynnessentialsllc@gmail.com">Email</a><a href="tel:+12132670825">Call</a></div><span>© {new Date().getFullYear()} Wynn Essentials. All rights reserved.</span></div>
    </footer>
