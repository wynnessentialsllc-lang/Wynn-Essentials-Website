...
import { getFreeShippingCampaign } from "../lib/annual-free-shipping";
...
const bohoHair: { name: string; image: string; alt: string; slug: string }[] = [
  { name: "Body Wave", image: "/collections/boho-card-body-wave.webp", alt: "Hand holding an 18-inch Wynn Essentials Body Wave human hair bulk bundle with product tag", slug: "boho-body-wave-18" },
  { name: "Bohemian Curl", image: "/collections/boho-card-bohemian-curl.webp", alt: "Hand holding an 18-inch Wynn Essentials Bohemian Curl human hair bulk bundle with product tag", slug: "boho-bohemian-curl-18" },
  { name: "Deep Wave", image: "/collections/boho-card-deep-wave.webp", alt: "Hand holding an 18-inch Wynn Essentials Deep Wave human hair bulk bundle with product tag", slug: "boho-deep-wave-18" },
  { name: "Spanish Curl", image: "/collections/boho-card-spanish-curl.webp", alt: "Hand holding an 18-inch Wynn Essentials Spanish Curl human hair bulk bundle with product tag", slug: "boho-spanish-curl-18" },
];
// Editorial photos for the floating photo wall behind the newsletter ("The Wynn
// Edit") sign-up. Each image is 560px wide; heights vary so the columns read as
// an organic collage. The section below duplicates each column's set so the slow
// vertical marquee loops seamlessly.
const newsletterGallery: { src: string; w: number; h: number }[] = [
  { src: "/newsletter/newsletter-nourish-robe.webp", w: 560, h: 981 },
  { src: "/newsletter/newsletter-wellness-lineup.webp", w: 560, h: 736 },
  { src: "/newsletter/newsletter-bubblegum-pigtails.webp", w: 560, h: 830 },
  { src: "/newsletter/newsletter-hair-therapy.webp", w: 560, h: 980 },
  { src: "/newsletter/newsletter-campaign-posters.webp", w: 560, h: 1107 },
  { src: "/newsletter/newsletter-hydrate-pink.webp", w: 560, h: 1218 },
  { src: "/newsletter/newsletter-oil-trio-pouch.webp", w: 560, h: 988 },
  { src: "/newsletter/newsletter-revaivl-mask.webp", w: 560, h: 686 },
  { src: "/newsletter/newsletter-hydrate-billboard.webp", w: 560, h: 984 },
  { src: "/newsletter/newsletter-hydrate-afro-purple.webp", w: 560, h: 315 },
];
...
function FirstOrderOffer({ onClose, onContinue, add, pool }: { onClose: () => void; onContinue: () => void; add: (p: Product) => void; pool: Product[] }) {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setState("sending");
    try {
      const r = await fetch("/api/subscribe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, consent, source: "first-order-popup" }) });
...
function Cart({ items, setItems, onClose, add, suggest, inStock }: { items: CartItem[]; setItems: (x: CartItem[]) => void; onClose: () => void; add: (p: Product) => void; suggest: Product[]; inStock: Product[] }) {
  ...
  // An item is checkout-ready if it has a chargeable price — a valid Stripe
  // price id, or (for variants billed via an inline price, like the Estate set)
  // a positive catalog price. Mirrors the server's create-checkout-session guard.
  const unconfigured = detailed.some(x => {
    const v = x.product.variants?.find(vv => vv.id === x.variantId);
    const priceId = v ? v.stripePriceId : x.product.stripePriceId;
    const price = v ? v.price : x.product.price;
    const hasValidPriceId = typeof priceId === "string" && /^price_[A-Za-z0-9]+$/.test(priceId);
    return !hasValidPriceId && !(price != null && price > 0);
  });
  ...
  const freeShippingCampaign = getFreeShippingCampaign();
  const campaignFreeShipping = !!freeShippingCampaign;
  return <ModalShell label="Shopping bag" onClose={onClose} className="drawer-shell"><aside className="drawer">
    ...
    {!items.length ? <div className="empty"><p>Your bag is ready for an intentional routine.</p><button className="button" onClick={onClose}>Continue Shopping</button></div> :
      ...
      <ul className="cart-trust"><li>Secure checkout by Stripe</li><li>{campaignFreeShipping ? `${freeShippingCampaign.label} free standard U.S. shipping` : `Free U.S. shipping over $${brandConfig.shippingThreshold}`}</li><li>Not loving it? Email us and we&rsquo;ll make it right.</li></ul>
...
// Compact rating shown on product cards. Renders nothing until a product has at
// least one review, so the grid never shows an empty "0.0" placeholder.
function CardRating({ summary, onClick }: { summary: ReviewSummary; onClick?: () => void }) {
  if (!summary.count) return null;
  const inner = <><Stars value={summary.average} /><span className="card-rating-num">{summary.average.toFixed(1)} <em>({summary.count})</em></span></>;
  return onClick
    ? <button type="button" className="card-rating" onClick={onClick} aria-label={`Rated ${summary.average} out of 5 from ${summary.count} review${summary.count === 1 ? "" : "s"}`}>{inner}</button>
    : <span className="card-rating">{inner}</span>;
}
...
function ProductDetail({ product, add, onClose, soldOut, submittedReviews, wished, onToggleWish }: { product: Product; add: (p: Product, qty?: number, color?: string, variantId?: string) => void; onClose: () => void; soldOut: boolean; submittedReviews: Review[]; wished: boolean; onToggleWish: () => void }) {
  ...
  const variantSoldOut = Boolean(selVariant?.soldOut) && !preorder;
  // The selected variant's own gallery replaces the product default so each
  // option (e.g. Uptown vs. Estate) shows its own photos as you toggle.
  const galleryImages = selVariant?.images?.length ? selVariant.images : (product.images ?? []);
  const variantLegend = product.variantLabel ?? "Length";
...
export default function WynnShop() {
  ...
  // When set (to a concern label from the "Shop by Concern" section), the shop grid
  // shows only products tagged with that concern, overriding the category filter.
  // The label must match the strings in each product's `concerns` array exactly.
  ...
  // "Complete your routine" cart cross-sell: in-stock haircare essentials not
  // already in the bag, ranked by how well they fit the routine steps you've
  // started, then by featured. Falls back to featured essentials for an empty
  // or hair-only bag.
  const recommendFor=(c:CartItem[])=>{
    const inCart=new Set(c.map(i=>i.slug));
    const steps=new Set(products.filter(p=>inCart.has(p.slug)&&!p.kind).map(p=>p.methodStep));
  ...
  const visible=(ingredientKeyword
    ...
    : concernFilter
    ? products.filter(p=>p.concerns.includes(concernFilter))
  ...
  return <div className="site">
    ...
    <main id="main">
      ...
      <section id="boho-hair" className="boho-hair section" aria-labelledby="boho-hair-heading"><div className="boho-editorial"><div className="boho-portrait"><Link href="/braiding-hair" aria-label="Shop Boho Hair"><figure><video src="/collections/boho-lifestyle-video.mov" poster="/collections/boho-lifestyle-braids.webp" width="900" height="1196" muted autoPlay loop playsInline preload="metadata" aria-hidden="true"/></figure></Link></div><div className="boho-intro"><p className="eyebrow">PREMIUM HUMAN HAIR</p><h2 id="boho-hair-heading"><span>Meet the Boho Hair</span><em>of Your</em><strong>DREAMS</strong></h2><div className="boho-sky-plane" aria-hidden="true"><svg viewBox="0 0 250 86" role="presentation"><defs><linearGradient id="bohoVaporFade" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#fff" stopOpacity=".18"/><stop offset=".48" stopColor="#fff" stopOpacity=".72"/><stop offset="1" stopColor="#fff" stopOpacity=".96"/></linearGradient><filter id="bohoVaporCloud" x="-20%" y="-60%" width="150%" height="220%"><feTurbulence type="fractalNoise" baseFrequency=".035 .12" numOctaves="2" seed="7" result="noise"/><feDisplacementMap in="SourceGraphic" in2="noise" scale="4"/><feGaussianBlur stdDeviation="2.8"/></filter></defs><path className="boho-vapor-mist" d="M4 68 C44 90 73 68 96 48 C112 34 126 35 148 42"/><path className="boho-vapor-body" d="M4 68 C44 90 73 68 96 48 C112 34 126 35 148 42"/><path className="boho-vapor-trail" d="M4 68 C44 90 73 68 96 48 C112 34 126 35 148 42"/></svg><img className="boho-sky-aircraft" src="/collections/boho-skywriting-plane-branded-v3.png" alt="" width="1501" height="1047"/></div><p className="boho-lede">Choose from four signature textures of premium 18-inch human hair, made to bring the lightweight, dimensional boho styles of your dreams to life.</p><Link className="boho-collection-link" href="/braiding-hair">Shop all braiding hair &amp; bundle guide →</Link><div className="boho-flourish" aria-hidden="true"><span>✦</span><span>✦</span><span>✦</span></div></div></div><div className="boho-product-heading"><p className="eyebrow">Four Signature Textures</p><h3>CHOOSE YOUR DREAM</h3><nav className="boho-texture-clouds" aria-label="Shop Boho Hair by texture"><Link href="/products/boho-body-wave-18">Body Wave</Link><Link href="/products/boho-bohemian-curl-18">Bohemian Curl</Link><Link href="/products/boho-deep-wave-18">Deep Wave</Link><Link href="/products/boho-spanish-curl-18">Spanish Curl</Link></nav></div><div className="boho-grid">
        {bohoHair.map(item=>{const product=products.find(p=>p.slug===item.slug)!;return <article className={`boho-card${soldOut(product)&&!isPreorderEligible(product.slug)?" is-sold-out":""}`} key={item.slug}><button className="art-button" onClick={()=>openProduct(product)} aria-label={`View ${item.name} details`}><div><img src={item.image} alt={item.alt} width="820" height="812" loading="lazy"/></div>{soldOut(product) && !isPreorderEligible(product.slug) && <span className="sold-out-badge">Sold Out</span>}</button><p className="eyebrow">BOHO BRAID HAIR</p><h3>{item.name}</h3><CardRating summary={ratingFor(product.slug)} onClick={()=>openProduct(product)}/><p>Premium human hair bulk with soft movement, natural blending, and braid-ready texture.</p><strong>{money(product.price)}</strong>{isPreorderEligible(product.slug) ? <button className="outline-button full sold-out-cta" onClick={()=>openProduct(product)}>PRE-ORDER</button> : soldOut(product) ? <button className="outline-button full sold-out-cta" onClick={()=>openProduct(product)}>Sold Out · Join the Waitlist</button> : <button className="outline-button full" onClick={()=>add(product)}>Add to Cart</button>}</article>;})}
      </div></section>
