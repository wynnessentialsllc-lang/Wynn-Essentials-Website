  ...
  return <ModalShell label="First-order offer" onClose={onClose} className="offer-shell">
    <div className="offer-modal">
      ...
      {state === "done" ? <div className="offer-done">
        ...
        {state === "err" && <p className="offer-err">Please enter a valid email and agree to emails.</p>}
...
function ProductDetail({ product, add, onClose, soldOut, submittedReviews, wished, onToggleWish }: { product: Product; add: (p: Product, qty?: number, color?: string, variantId?: string) => void; onClose: () => void; soldOut: boolean; submittedReviews: Review[]; wished: boolean; onToggleWish: () => void }) {
  ...
  return <ModalShell label={`${product.name} product details`} onClose={onClose} className="product-shell"><article className="product-modal">
    ...
    <div className="product-gallery">{product.video && <div className="product-art product-photo product-video" key="video"><video src={product.video} muted loop playsInline autoPlay preload="metadata" aria-label={`${product.name} product video`}/></div>}{(galleryImages.length ? galleryImages : [null, null]).map((image,index)=>image ? <div className="product-art product-photo" key={image.src}><img src={image.src} alt={image.alt} width="1600" height="1600" loading={index ? "lazy" : undefined}/></div> : <ProductArt product={product} key={index}/>)}</div>
