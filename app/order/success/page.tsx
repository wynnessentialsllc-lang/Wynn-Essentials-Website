...
export default async function OrderSuccess({ searchParams }: { searchParams: Promise<{ session_id?: string }> }) {
  ...
  const includesPreorder = session.line_items?.data.some(item => item.description?.includes("PRE-ORDER"));
  return <main className="order-page"><SuccessClient confirmed={confirmed} value={session.amount_total == null ? null : session.amount_total / 100} currency={(session.currency || "usd").toUpperCase()} orderRef={session.metadata?.internalOrderReference || session.id}/><p className="eyebrow">WYNN ESSENTIALS ORDER</p><h1>{confirmed ? "Your order is confirmed." : "Your payment is processing."}</h1><p>{confirmed ? "We’ve sent your receipt and order details to your email." : "We’ll confirm your order after Stripe reports a successful payment."}</p>{includesPreorder && <div className="product-preorder"><p className="preorder-label">PRE-ORDER</p><h2>Pre-order details</h2><p>Pre-orders close every Friday at 12 PM PT.</p><p>Order before the cutoff to be included in the current pre-order batch.</p><p>Please allow approximately 7–13 days for processing before shipment.</p></div>}<dl><div><dt>Order reference</dt><dd>{session.metadata?.internalOrderReference || "Pending"}</dd></div><div><dt>Email</dt><dd>{session.customer_details?.email || "Provided securely at checkout"}</dd></div><div><dt>Amount paid</dt><dd>{session.amount_total == null ? "Pending" : new Intl.NumberFormat("en-US",{style:"currency",currency:(session.currency||"usd").toUpperCase()}).format(session.amount_total/100)}</dd></div>{session.line_items?.data.map(item=><div key={item.id}><dt>{item.description}</dt><dd>Quantity {item.quantity}</dd></div>)}</dl><div className="actions"><Link className="button" href="/">Continue Shopping</Link><Link className="outline-button" href="/#ingredients">View Hair Care Guidance</Link></div></main>;
}
